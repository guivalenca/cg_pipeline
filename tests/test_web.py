...
def test_shell_exposes_only_the_two_product_pages():
    shell = (
    ...
    assert "{ path: '/syllabi', label: 'Syllabi' }" in shell
    assert "{ path: '/graph', label: 'Graph Visualization' }" in shell
    for retired in ("/sources", "/runs", "/structure", "/universe"):
        assert retired not in shell


def test_source_actions_keep_manual_access_visible_and_adapter_gating_conservative():
    script = (
        Path(__file__).resolve().parents[1]
        / "src"
        / "universe"
    ...
    assert "if (kind === 'article' || kind === 'artigo')" in script
    assert "capability.supported" in script
    assert "Adapter de ${mediaLabel(source).toLowerCase()} indisponível" in script
    assert "['queued', 'running', 'downloaded'].includes(image.status)" in script
    assert "analysis.ocr || analysis.visible_text" in script
    assert "source.pipeline = { status: jobStatus === 'queued' ? 'queued' : 'extracting' }" in script
    assert "source.has_markdown = false" in script
    assert (
