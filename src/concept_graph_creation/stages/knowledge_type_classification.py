...
def run_knowledge_type_classification_phase(
    *,
    ...
    if not subject_merge_path.is_file():
        raise StageBlockedError("Knowledge Type Classification requires subject_merge.json from 06-subject-merge")
    if not segmentation_summary_path.is_file():
        raise StageBlockedError(
            "Knowledge Type Classification requires lesson_segmentation_summary.json from 08-lesson-segmentation"
        )
    ...
    execution_identity = model_execution_identity(
        ...
        schema_version="knowledge_type_execution.v1",
        extra_config={"quality_repair_round_limit": 3},
    )
    classification_execution_identity = model_execution_identity(
        router=resolved_router,
        model_call=model_call,
        routes={
            "classification": classification_model_route,
        ...
        provider_retry_limit=provider_retry_limit,
        provider_retry_backoff_seconds=provider_retry_backoff_seconds,
        schema_version="knowledge_type_classification_execution.v1",
    )
    audit_execution_identity = model_execution_identity(
        router=resolved_router,
        model_call=model_call,
        routes={"audit": audit_model_route, "format_repair": format_repair_model_route},
        provider_retry_limit=provider_retry_limit,
        provider_retry_backoff_seconds=provider_retry_backoff_seconds,
        schema_version="knowledge_type_audit_execution.v1",
    )
    repair_execution_identity = model_execution_identity(
        router=resolved_router,
        model_call=model_call,
        routes={
            "quality_repair": quality_repair_model_route,
        ...
        provider_retry_limit=provider_retry_limit,
        provider_retry_backoff_seconds=provider_retry_backoff_seconds,
        schema_version="knowledge_type_repair_execution.v1",
    )
    ...
    segmentation_summary_revision = canonical_revision(
        ...
        progress_callback(
            {
                ...
                "details": {**(details or {}), "progress_unit": "operações"},
            }
        )
    ...
    def on_segment_progress(event: dict[str, Any]) -> None:
        nonlocal progress_current
        progress_current = int(event["current"])
        segment_details = dict(event.get("details") or {})
        segment_label = str(
            segment_details.get("segment_label")
            or segment_details.get("completed_segment_id")
            or ""
        )
        report_progress(
            message=(
                f"{progress_current}/{progress_total} operações concluídas · "
                f"último: Segmento {segment_label}"
            ),
            details={
                **segment_details,
                "completed_item_type": "Segmento",
                "completed_item_id": str(segment_details.get("completed_segment_id") or segment_label),
                "active_stage": "segment_classification",
                "stage_current": int(event["current"]),
                "stage_total": int(event["total"]),
            },
        )

    def on_segment_activity(event: dict[str, Any]) -> None:
    ...
    segment_results = _run_segment_classification_tasks(
        ...
        provider_retry_limit=provider_retry_limit,
        provider_retry_backoff_seconds=provider_retry_backoff_seconds,
        execution_identity=classification_execution_identity,
        progress_callback=on_segment_progress,
        activity_callback=on_segment_activity,
    )
    ...
    first_audit_result = _run_quality_audit(
        run_dir=run_dir,
        aggregate=aggregate,
        taxonomy=taxonomy,
        ...
        provider_retry_limit=provider_retry_limit,
        provider_retry_backoff_seconds=provider_retry_backoff_seconds,
        execution_identity=audit_execution_identity,
        upstream_derivation_keys=[result.derivation_key for result in segment_results],
    )
    ...
    advance_progress(
        item_type="Auditoria",
        item_id=str(audit_count),
        stage="quality_audit",
        details={"reliability": audit.get("reliability")},
    )

    repair_artifact_path: Path | None = None
    repair_results: list[DerivedArtifact] = []
    ...
    while audit["reliability"] == "repair_required" and repair_count < max_quality_repair_rounds:
        ...
        report_progress(
            message=(
                f"{progress_current}/{progress_total} operações concluídas · "
                f"reparo {repair_count + 1} preparado"
            ),
            details={
                "active_stage": "quality_repair",
                "repair_round": repair_count + 1,
                "target_concept_count": len(target_concept_ids),
            },
        )
        repair_result = _run_quality_repair(
            run_dir=run_dir,
            aggregate=aggregate,
            ...
            provider_retry_limit=provider_retry_limit,
            provider_retry_backoff_seconds=provider_retry_backoff_seconds,
            execution_identity=repair_execution_identity,
            upstream_derivation_key=final_audit_result.derivation_key,
        )
        repair_results.append(repair_result)
        repair_artifact_path = run_dir / "knowledge_type_quality_repair_decision.json"
        aggregate = _apply_quality_repair(
            aggregate=aggregate,
            repair_decision=repair_result.artifact,
        ...
        advance_progress(
            item_type="Reparo",
            item_id=str(repair_count),
            stage="quality_repair",
            details={"target_concept_count": len(target_concept_ids)},
        )
        final_audit_result = _run_quality_audit(
            run_dir=run_dir,
            aggregate=aggregate,
            taxonomy=taxonomy,
            prompt=prompts["quality_audit"],
            ...
            provider_retry_backoff_seconds=provider_retry_backoff_seconds,
            execution_identity=audit_execution_identity,
            upstream_derivation_keys=[
                *[result.derivation_key for result in segment_results],
                *[result.derivation_key for result in repair_results],
            ],
        )
    ...
    run_identity = {
        "schema_version": "knowledge_type_derivation.v1",
        "source_ledger_revision": source_ledger_revision,
        "subject_merge_revision": subject_merge_revision,
        "lesson_segmentation_summary_revision": segmentation_summary_revision,
        "lesson_segments_revisions": [task.lesson_segments_revision for task in tasks],
        "taxonomy_revision": canonical_revision(taxonomy, ignored_keys=VOLATILE_DERIVATION_KEYS),
        ...
        "contracts_revision": canonical_revision(
            {
                "classification": _classification_output_contract(),
                "audit": _quality_audit_output_contract(),
                "repair": _classification_output_contract(),
            }
        ),
        "segment_derivation_keys": [result.derivation_key for result in segment_results],
        "execution": execution_identity,
    }
    run_key = derivation_key(run_identity)
    summary_artifact["derivation"] = {
        "schema_version": "knowledge_type_derivation.v1",
        "status": "succeeded",
        "key": run_key,
        "completed_at": _now(),
        "identity": run_identity,
        "output_revision": _knowledge_type_summary_output_revision(summary_artifact),
        "segment_derivations": [
            {
                "artifact": result.derived_artifact_path_ref,
        ...
        "repair_derivations": [
            {
                "artifact": str(result.artifact_path.relative_to(run_dir)),
                "derivation_key": result.derivation_key,
            }
        ...
        for segment in lesson_segments.get("segments") or []:
            ...
            tasks.append(
                _SegmentClassificationTask(
                    ...
                    source_ledger_revision=source_ledger_revision,
                    subject_merge_revision=subject_merge_revision,
                    lesson_segments_revision=canonical_revision(
                        lesson_segments,
                        ignored_keys=VOLATILE_DERIVATION_KEYS,
                    ),
                )
...
def _run_segment_classification_tasks(
    *,
    ...
    return run_bounded_model_work(
        tasks,
        worker=lambda task, control: _run_segment_classification_task(
            task=task,
            runner=runner,
            prompt=prompt,
            taxonomy=taxonomy,
        ...
        concurrency=concurrency,
        on_result=lambda result, current, total: _emit_knowledge_type_progress(
            progress_callback,
            current=current,
            total=total,
            result=result,
        ...
        on_activity=(
            lambda activity: activity_callback(
                safe_model_work_activity(
                    ...
                    item_type="Segmento",
                    item_ids=[
                        f"{task.lesson_id} / {task.segment_label or task.segment_id}"
                        for task in tasks
                    ],
                )
            )
...
def _emit_knowledge_type_progress(
    callback: Callable[[dict[str, Any]], None] | None,
    *,
    current: int,
    total: int,
    result: _SegmentClassificationResult | None = None,
) -> None:
    if callback is None:
        return
    details: dict[str, Any] = {"progress_unit": "segmentos"}
    message = f"{current}/{total} segmentos classificados"
    if result is not None:
        details.update(
            {
                "completed_lesson_id": result.lesson_id,
                "completed_segment_id": result.segment_id,
                "segment_label": result.segment_label,
                "repaired": result.repaired,
            }
        )
        last_item = result.segment_label or result.segment_id
        message += f" · último: {last_item}"
    callback(
        {
            "current": current,
            "total": total,
...
def _run_segment_classification_task(
    *,
    ...
    model_input = {
        ...
        "ledger_fingerprint": task.current_ledger_fingerprint,
        "source_artifacts": {
    ...
    identity = stage_derivation_identity(
        ...
        input_revisions={
            ...
            "subject_merge": task.subject_merge_revision,
            "lesson_segments": task.lesson_segments_revision,
            "lesson": canonical_revision(task.lesson, ignored_keys=VOLATILE_DERIVATION_KEYS),
            "concepts": canonical_revision(task.concepts, ignored_keys=VOLATILE_DERIVATION_KEYS),
        },
    )
    result = run_cached_stage(
        cache_root=task.segment_dir / "knowledge_type_derivations",
        stage_group="classifications",
        input_filename="knowledge_type_classification_input.json",
        output_filename="knowledge_type_classification.json",
        model_input=model_input,
        identity=identity,
        validator=validator,
    ...
    compatibility_input_path = task.segment_dir / "knowledge_type_classification_input.json"
    compatibility_artifact_path = task.segment_dir / "knowledge_type_classification.json"
    _write_compatibility_artifact(compatibility_input_path, model_input)
    _write_compatibility_artifact(compatibility_artifact_path, result.artifact)
    return _SegmentClassificationResult(
        ...
        artifact_path_ref=str(compatibility_artifact_path.relative_to(task.run_dir)),
        artifact=result.artifact,
        repaired=result.repaired,
        derivation_key=result.derivation_key,
        derived_artifact_path_ref=str(result.artifact_path.relative_to(task.run_dir)),
    )
...
def _run_quality_audit(
    *,
    ...
    provider_retry_backoff_seconds: float,
    execution_identity: dict[str, Any],
    upstream_derivation_keys: list[str],
) -> DerivedArtifact:
    model_input = {
    ...
    identity = stage_derivation_identity(
        ...
        execution_identity=execution_identity,
        input_revisions={
            "aggregate": canonical_revision(
                aggregate,
                ignored_keys=VOLATILE_DERIVATION_KEYS,
            ),
            "upstream_derivation_keys": upstream_derivation_keys,
            "taxonomy": canonical_revision(
                taxonomy,
                ignored_keys=VOLATILE_DERIVATION_KEYS,
            ),
        },
    ...
    result = run_cached_stage(
        cache_root=run_dir / "knowledge_type_derivations",
        stage_group="audits",
        input_filename="knowledge_type_quality_audit_input.json",
        output_filename="knowledge_type_quality_audit.json",
        model_input=model_input,
        identity=identity,
        validator=_validate_quality_audit,
        execute=lambda stage_dir: _run_stage_with_provider_retry(
        ...
        postprocess=lambda artifact: _apply_quality_guardrails(artifact, aggregate=aggregate),
    )
    _write_compatibility_artifact(run_dir / "knowledge_type_quality_audit_input.json", model_input)
    _write_compatibility_artifact(run_dir / "knowledge_type_quality_audit.json", result.artifact)
    return result
...
def _run_quality_repair(
    *,
    ...
    identity = stage_derivation_identity(
        stage_name="knowledge_type_quality_repair",
        model_input=model_input,
        execution_identity=execution_identity,
        input_revisions={
            "aggregate": canonical_revision(
    ...
    result = run_cached_stage(
        cache_root=run_dir / "knowledge_type_derivations",
        stage_group="repairs",
        input_filename="knowledge_type_quality_repair_input.json",
        output_filename="knowledge_type_quality_repair_decision.json",
        model_input=model_input,
        identity=identity,
        validator=validator,
        execute=lambda stage_dir: _run_stage_with_provider_retry(
    ...
    _write_compatibility_artifact(run_dir / "knowledge_type_quality_repair_input.json", model_input)
    _write_compatibility_artifact(run_dir / "knowledge_type_quality_repair_decision.json", result.artifact)
    return result
...
def _knowledge_type_summary_output_revision(artifact: dict[str, Any]) -> str:
...
def _normalize_classification_output(
    ...
    *,
    ...
    if model_input.get("ledger_fingerprint"):
        stamp_ledger_fingerprint(artifact, model_input["ledger_fingerprint"])
    return artifact
...
def _load_knowledge_type_taxonomy() -> dict[str, Any]:
    creation_root = Path(__file__).resolve().parents[3]
    system_prompt_path = creation_root / "prompts" / "knowledge_type_classification" / "taxonomy_source.txt"
    ...
    return {
        "source_artifact": _path_ref(system_prompt_path, creation_root),
