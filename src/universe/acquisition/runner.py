...
def _oldest_ready_work_kind(conn: psycopg.Connection) -> str | None:
    ...
    row = conn.execute(
        ...
        "   OR (status = 'running' AND lease_expires_at < now()))"
        " UNION ALL"
        " SELECT 'lesson_build'::text AS kind, work.available_at, work.created_at, work.id"
        " FROM lesson_build_work work"
        " JOIN lesson_build build ON build.id = work.build_id"
        " WHERE work.available_at <= now() AND work.status IN ('queued', 'running')"
        "   AND work.seq = 1 AND build.is_active"
        "   AND (work.claim_token IS NULL OR work.lease_expires_at <= now())"
        " UNION ALL"
