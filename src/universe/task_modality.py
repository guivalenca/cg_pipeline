"""Judge whether each task demands doing something or explaining something.

    python -m universe.task_modality run --prompt v001 --model deepseek/deepseek-v4-pro \
        --gen-runs r0052 --tool prompts/task-modality/tool-v001.json

One blind call per task carries only the task and answer, never the source. It
judges whether the task demands doing or explaining; unsure remains a distinct
verdict.
"""
...
def modality_of(item: dict) -> dict | str:
    """The task modality and reason, or why neither is usable.

    One blind call per task judges whether the task demands doing or explaining.
    """
    if item["error"]:
        return "error"
    try:
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    statements = [modality_of(item) for item in items]
    tally = {
        label: sum(
            1
        ...
        for label in ("do", "explain", "unsure", "error", "unparseable")
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="universe.task_modality", description=__doc__)
    sub = parser.add_subparsers(dest="command", required=True)

    run = sub.add_parser("run", help="judge whether every task demands doing or explaining")
