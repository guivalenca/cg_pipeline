...
def select_targets(
    ...
    where = "WHERE s.id = ANY(%s)" if source_ids else ""
    rows = conn.execute(
...
def execute(
    ...
    conn.execute(
        ...
        (run_id, stage, client.model, prompt.ref, prompt.sha, Jsonb(client.params)),
    )
    conn.commit()
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    with connect() as conn:
        ...
        targets = select_targets(conn, sources, None if args.all else args.limit)
        if not targets:
        ...
        summary = execute(conn, args.stage, prompt, client, targets)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    selection.add_argument("--sources", help="comma-separated source ids")
    selection.add_argument("--limit", type=int, help="first N sources by id")
