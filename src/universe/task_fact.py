"""Judge whether the knowledge each task asks a learner to demonstrate is a fact.

    python -m universe.task_fact run --prompt v001 --model deepseek/deepseek-v4-pro \
        --gen-runs r0052 --tool prompts/task-fact/tool-v001.json

One blind call per task carries only the task and answer, never the source. It
classifies the knowledge as a fact or not_fact.
"""
...
STAGE = "task-fact"
VERDICTS = {"fact", "not_fact"}
TRIAGE_VERDICTS = {"supported", "unsupported", "unsure"}
DEFAULT_WORKERS = 4


def fact_of(item: dict) -> dict | str:
    """The fact verdict and reason, or why neither is usable."""
    if item["error"]:
        return "error"
    try:
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    statements = [fact_of(item) for item in items]
    tally = {
        label: sum(
            1
        ...
        for label in ("fact", "not_fact", "error", "unparseable")
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="universe.task_fact", description=__doc__)
    sub = parser.add_subparsers(dest="command", required=True)

    run = sub.add_parser("run", help="classify whether the knowledge each task demands is a fact")
