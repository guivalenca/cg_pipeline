...
| - | - | - | - |
| pdf-parse | Firecrawl `/v2/parse`, PDF `auto` mode, `markdown` + `images` | firecrawl-v2-parse.v1 | 2026-08-10: Firecrawl Markdown is canonical so tables remain tables and reading order is provider-owned. Extracted figures become local `pdf_figure` assets. Poppler text and 144 dpi page renders remain audit-only; no full-page Gemini selection runs. |
| pdf-figure-localization | gemini-2.5-flash via OpenRouter, candidate pages only | pdf-figure-localization/v002, named bounding-box tool | 2026-08-10: Firecrawl omitted the vector Figure 1. V002 names each coordinate and asks for the entire connected visual. Poppler caption geometry then extends an undersized lower edge locally to just before the figure caption; model and adjusted boxes remain auditable. |
| source-image-analysis | gemini-2.5-flash via OpenRouter, one grouped call per source | source-image-analysis/v001, tool-v001 | 2026-08-07 initial rollout; 29 candidates in the first real source, 23 analyzed in one call, 12 retained, 11 omitted, 6 technical failures preserved unresolved; US$0.0156393 |
| passage-cleanup triage | deepseek-v4-flash, thinking high | passage-triage/v004, tool-v003 (tool-v003-atomic for one-element passages) | 2026-08-10: v004 makes citation-only bibliographies explicit filler while preserving citations attached to teaching. PDF pipeline artifacts also apply a structural policy that preserves every enriched page-image atom and audits a conflicting model decision as `primary_enriched_image_preserved`; this followed an acceptance run that dropped page 7 of a two-page table. v004 supersedes v003 after the first page-aware PDF acceptance run kept a six-page reference list. v003 initial rollout produced 8 keep / 10 drop / 1 unknown in local syllabus runs r0002+r0004, including two revised passages; exact-state report at reports/passage-triage-r0002-r0004.md |
...
| task-modality | deepseek-v4-pro, thinking high, single call (majority-of-3 retired 2026-08-02) | task-modality/v002, tool-v001 | 2026-08-02: voting retired by the v1 simplification — one call per axis going forward. Identity no longer rides on axes (the directional judge tests transfer directly; axes now filter judge candidates and inform segment teaching style), so single-call noise is acceptable and recomputable. Existing voted runs stand; no re-run. Prior record: 2026-07-28, r0124+r0125+r0126 under universe.task_axes; v002 draws the line at apply-to-a-case vs put-into-words (Merrill use/remember, KLI new-instance), so retrieval-shaped "Name X" tasks are explain; 31/33 unanimous. v001 (r0109-r0111) retired with the fact scoping |
...
| kc-judge (directional mastery) | deepseek-v4-pro, thinking high, 16 workers | kc-judge/v002-surmise-pair, tool-v002 (both directions in one call, 4-level per direction) — decided 2026-08-02 by founder over flash (5x cheaper, zero false merges, but surfaces far fewer mutual flags; flags are information). Bench: reports/kc-judge-bench-v002.md; $0.40/source measured. Prior default v001-surmise/tool-v001 (one direction per call) retired with the v002 bench. | 2026-08-01, scratch bench over the r0130 corpus (v001 bench records removed in the 2026-08-02 repo cleanup; the v002 raw run data lives in the research archive at ~/Desktop/concept-universe-research/). Theory-framed prompt beat the lean control (v001-transfer): 6 composite KCs vs 3, Opus siding with surmise on all 3 disputed groups; lean prompt failed by literalism (task-wording accidents read as knowledge demands). Candidate set: generator union (semantic floor 0.70 cap 15 + lexical BM25 top-5 + closure), floor confirmed by yield curve — duplas exist down to 0.72, none below. Middle-level hedging low (10.6%). Known weakness: contested "likely" yeses lean wrong (analysis: opus-vs-deepseek-r0130.md in ~/Desktop/concept-universe-research/); remedy open — the earlier likely-gated-escalation idea was dropped by founder decision 2026-08-01 (no second-model tier in the pipeline). OpenRouter routing for future runs: exclude fp4 providers, sort by throughput; provider stamped per call since 2026-08-01. 2026-08-02 (v1 simplification, benched same day — reports/kc-judge-bench-v002.md): v002-surmise-pair + tool-v002 judge both directions in ONE call per pair; candidate generation keeps floor 0.70 (founder decision — real doubles exist down to 0.72), cap 6, lexical top-5, axis filter applied after the cap (an axis-incompatible neighbor spends a slot and is then dropped; founder accepted the trade), legacy/dense generators off; merges count clear_yes-both-directions only ("likely" no longer merges); the 2026-08-01 routing decision (sort by throughput, no low-bit quantization) is now wired into the runner's request defaults; flash was benched alongside pro and the founder settled on pro. On the reference corpus this is 100 pairs = 100 calls, catching 11/11 known same-axis doubles (9 further known doubles cross axes and are excluded by the axis rule itself). |
